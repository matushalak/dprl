...
def parse_args():
    ...
    parser.add_argument('-ipm', type = int, default = 100) # iterations per move
...
if __name__ == '__main__':
    ...
    w, winB, Tree = game(B, opponent=args.mode, symbols = d, PRINT=True, 
                         random_enemy_search= not args.minimax,
                         VISUALIZE = args.convergence, iter_per_turn=args.ipm)
    ...
    if args.mode in {'random', 'optimal'} and not args.convergence and not args.WINconvergence:
        results = parellel_simulate(B, args.mode, d, args.nsim, nominimax= not args.minimax, ipm = args.ipm)
